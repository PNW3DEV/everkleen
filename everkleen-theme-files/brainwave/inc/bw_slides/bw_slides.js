...
(function( $ ) {
    ...
    redux.field_objects.bw_slides = redux.field_objects.bw_slides || {};

    $( document ).ready(
        function() {
            //redux.field_objects.bw_slides.init();
        }
    );

    redux.field_objects.bw_slides.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-bw_slides:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                if ( parent.hasClass( 'redux-container-bw_slides' ) ) {
                ...
                el.find( '.redux-slides-remove' ).live(
                    'click', function() {
                        ...
                        var slideCount = $( this ).parents( '.redux-container-bw_slides:first' ).find( '.redux-slides-accordion-group' ).length;
                        ...
                        if ( slideCount > 1 ) {
                            ...
                            $( this ).parents( '.redux-container-bw_slides:first' ).find( '.redux-slides-accordion-group:last' ).find( '.redux-slides-header' ).text( content_new_title );
                        }
                    }
                );
