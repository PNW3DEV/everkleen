...
// Don't duplicate me!
if ( ! class_exists( 'ReduxFramework_bw_slides' ) ) {

	/**
	 * Main ReduxFramework_bw_slides class
	 *
	 * @since       1.0.0
	 */
	class ReduxFramework_bw_slides extends ReduxFramework {
		...
		public function render() {
			...
			echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr( sprintf( __( '%s', 'redux-framework' ), $this->field['content_title'] ) ) . '">';
			...
			if ( isset ( $this->value ) && is_array( $this->value ) && ! empty ( $this->value ) ) {
				...
				foreach ( $slides as $slide ) {
					...
					echo '<div class="redux-slides-accordion-group"><fieldset class="redux-field" data-id="' . esc_attr( $this->field['id'] ) . '"><h3><span class="redux-slides-header">' . $slide['title'] . '</span></h3><div>';
					...
					echo '<span class="button remove-image' . esc_attr( $hide ) . '" id="reset_' . $x . '" rel="' . esc_attr( $slide['attachment_id'] ) . '">' . __( 'Remove', 'redux-framework' ) . '</span>';
					...
					echo '<li><input type="' . $title_type . '" id="' . esc_attr( $this->field['id'] ) . '-title_' . $x . '" name="' . $this->field['name'] . '[' . $x . '][title]' . $this->field['name_suffix'] . '" value="' . esc_attr( $slide['title'] ) . '" placeholder="' . $placeholder . '" class="full-text slide-title" /></li>';
					...
					if ( $this->field['show']['description'] ) {
						...
						$temp_id     = $this->field['name'] . '[' . $x . '][description]' . $this->field['name_suffix'];
						$desc        = $slide['description'];
						echo '<li>' . @wp_editor( $desc, $temp_id ) . '</li>';
					}
			...
			if ( $x == 0 ) {
				echo '<div class="redux-slides-accordion-group"><fieldset class="redux-field" data-id="' . $this->field['id'] . '"><h3><span class="redux-slides-header">' . esc_attr( sprintf( __( '%s', 'redux-framework' ), $this->field['content_title'] ) ) . '</span></h3><div>';
		...
		public function enqueue() {
			...
			if ( $this->parent->args['dev_mode'] ) {
				...
				wp_enqueue_style(
					'redux-field-bw_slides-css',
					get_template_directory_uri() . '/inc/bw_slides/bw_slides.css',
					array(),
					time(),
					'all'
				);
			...
			wp_enqueue_script(
				'redux-field-bw_slides-js',
				get_template_directory_uri() . '/inc/bw_slides/bw_slides.js',
				array(
					'jquery',
					'jquery-ui-core',
					'jquery-ui-accordion',
