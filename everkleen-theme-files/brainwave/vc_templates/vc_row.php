...
if ( ! empty( $atts['page_section'] ) ) {
	switch ( $atts['page_section'] ) {
		case 'None':
			break;
		case 'Small':
			$css_classes[] = 'page-section-small';
			break;
		case 'Normal':
...
if ( strpos( $css, 'url' ) ) {
	$parts = explode( ' ', $css );
	if ( strpos( $parts[1], 'rgba' ) === 0 ) {
		$output .= "<div class=\"overlay\" style=\"background: {$parts[1]}; z-index:0\"></div>";
	}
}
if ( ! empty( $css ) ) {
